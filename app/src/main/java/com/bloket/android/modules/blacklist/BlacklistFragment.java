...
public class BlacklistFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater mInflater, @Nullable ViewGroup mContainer, @Nullable Bundle mSavedInstance) {
        ...
        mAdapter = new BlacklistAdapter(getContext(), mDbHelper.getBlacklistData());
    ...
    public void onActivityResult(int mRequestCode, int mResultCode, Intent mData) {
        ...
        if (mRequestCode == ContactPickerActivity.PICKER_REQUEST_CODE) {
            ...
            BlacklistTask mAddTask = new BlacklistTask(getContext(), mSelectedContacts, mSuccess -> {
                if (mSuccess) {
                    mAdapter = new BlacklistAdapter(getContext(), mDbHelper.getBlacklistData());
                    mRecyclerView.setAdapter(mAdapter);
                }
    ...
    private void showAddDialog() {
        ...
        mAddContact.setOnClickListener(mView -> {
            ...
            mIntent.putParcelableArrayListExtra("PRE_SELECT_LIST", mDbHelper.getBlacklistContacts());
        ...
        mAddNumber.setOnClickListener(mView -> {
            ...
            mPositive.setOnClickListener(mButtonView -> {
                ...
                if (mPattern.matcher(mNumber).matches()) {
                    if (mDbHelper == null) mDbHelper = new DatabaseHelper(getContext());
                    if (!mDbHelper.chkBlacklistNumber(mNumber)) {
                        mDbHelper.putBlacklistData(DatabaseHelper.TYPE_NUMBER, DatabaseHelper.DEFAULT_GROUP_ID, "", "", mNumber, "", "", "", "");
                        mAdapter = new BlacklistAdapter(getContext(), mDbHelper.getBlacklistData());
                        mRecyclerView.setAdapter(mAdapter);
                        UIHelper.hideSoftKeyboard(getContext(), mPhoneNumber);
                        mDialog.dismiss();
                    } else {
                        mPhoneNumber.setError("Number is already stored in blacklist");
                    }
                } else {
        ...
        mAddStarts.setOnClickListener(mView -> {
            ...
            mPositive.setOnClickListener(mButtonView -> {
                ...
                if (mPattern.matcher(mNumber).matches()) {
                    if (mDbHelper == null) mDbHelper = new DatabaseHelper(getContext());
                    if (!mDbHelper.chkBlacklistStarts(mNumber)) {
                        mDbHelper.putBlacklistData(DatabaseHelper.TYPE_STARTS, DatabaseHelper.DEFAULT_GROUP_ID, "", "", mNumber, "", "", "", "");
                        mAdapter = new BlacklistAdapter(getContext(), mDbHelper.getBlacklistData());
                        mRecyclerView.setAdapter(mAdapter);
                        UIHelper.hideSoftKeyboard(getContext(), mPhoneNumber);
                        mDialog.dismiss();
                    } else {
                        mPhoneNumber.setError("Entry is already stored in blacklist");
                    }
                } else {
                    mPhoneNumber.setError("Enter a valid number");
                }
        ...
        mAddEnds.setOnClickListener(mView -> {
            ...
            mPositive.setOnClickListener(mButtonView -> {
                ...
                if (mPattern.matcher(mNumber).matches()) {
                    if (mDbHelper == null) mDbHelper = new DatabaseHelper(getContext());
                    if (!mDbHelper.chkBlacklistEnds(mNumber)) {
                        mDbHelper.putBlacklistData(DatabaseHelper.TYPE_ENDS, DatabaseHelper.DEFAULT_GROUP_ID, "", "", mNumber, "", "", "", "");
                        mAdapter = new BlacklistAdapter(getContext(), mDbHelper.getBlacklistData());
                        mRecyclerView.setAdapter(mAdapter);
                        UIHelper.hideSoftKeyboard(getContext(), mPhoneNumber);
                        mDialog.dismiss();
        ...
        mAddContains.setOnClickListener(mView -> {
            ...
            mPositive.setOnClickListener(mButtonView -> {
                ...
                if (mPattern.matcher(mNumber).matches()) {
                    if (mDbHelper == null) mDbHelper = new DatabaseHelper(getContext());
                    if (!mDbHelper.chkBlacklistContains(mNumber)) {
                        mDbHelper.putBlacklistData(DatabaseHelper.TYPE_CONTAINS, DatabaseHelper.DEFAULT_GROUP_ID, "", "", mNumber, "", "", "", "");
                        mAdapter = new BlacklistAdapter(getContext(), mDbHelper.getBlacklistData());
                        mRecyclerView.setAdapter(mAdapter);
                        UIHelper.hideSoftKeyboard(getContext(), mPhoneNumber);
                        mDialog.dismiss();
            ...
            mPositive.setOnClickListener(mButtonView -> {
                ...
                if (mPattern.matcher(mStartNumber).matches() && mPattern.matcher(mEndNumber).matches()) {
                    ...
                    if (mNumberOne < mNumberTwo) {
                        if (mDbHelper == null) mDbHelper = new DatabaseHelper(getContext());
                        if (!mDbHelper.chkBlacklistRange(String.valueOf(mNumberOne), String.valueOf(mNumberTwo))) {
                            mDbHelper.putBlacklistData(DatabaseHelper.TYPE_RANGE, DatabaseHelper.DEFAULT_GROUP_ID, "", "", String.valueOf(mNumberOne), String.valueOf(mNumberTwo), "", "", "");
                            mAdapter = new BlacklistAdapter(getContext(), mDbHelper.getBlacklistData());
                            mRecyclerView.setAdapter(mAdapter);
                            UIHelper.hideSoftKeyboard(getContext(), mPositive);
                            mDialog.dismiss();
                        } else {
                            mRangeEnd.setError("Entry is already stored in blacklist");
                        }
                    } else {
