...
public class WhitelistTask extends AsyncTask<Void, Void, Void> {

    @SuppressLint("StaticFieldLeak")
    private Context mContext;
    private ArrayList<ContactPickerResult> mSelectedContacts;
    private Dialog mProgressDialog;
    private WhitelistAddResponse mResponse;

    WhitelistTask(Context mContext, ArrayList<ContactPickerResult> mSelectedContacts, WhitelistAddResponse mResponse) {
    ...
    protected void onPreExecute() {
        ...
        mProgressDialog.setOnCancelListener(mDialog -> WhitelistTask.this.cancel(true));
    ...
    protected Void doInBackground(Void... mVoid) {
        ...
        ArrayList<ContactPickerIdNumber> mOldSelection = mDbHelper.getWhitelistContacts();
        ...
        for (ContactPickerIdNumber mContact : mOldSelection) {
            ...
            mDbHelper.delWhitelistContact(Integer.valueOf(mContact.getContactId()), mContact.getPhoneNumber());
        }
        for (ContactPickerIdNumber mContact : mNewSelection) {
            if (mContact.getContactId() == null) continue;
            mDbHelper.putWhitelistData(DatabaseHelper.TYPE_CONTACT, DatabaseHelper.DEFAULT_GROUP_ID, "", mContact.getContactId(), mContact.getPhoneNumber(), "", "", "");
        }
        return null;
    }
    ...
    public interface WhitelistAddResponse {
